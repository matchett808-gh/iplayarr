...
function episodeTitleMatcher(match, epname, s,e ) {
  ...
  return (process(match[3]) === process(epname) || match[3] == `Episode ${zeroPad(e, 2)}` || match[3] == `Episode ${e}`) && seriesMatch(match[0], s);
}
...
app.get('/sonarr', (req, res) => {
  ...
  if (req.query.t === 'caps') {
    ...
    axios.get(url).then((showres) => {
      ...
      /* Overwrite the SeriesPid with the one from the map */
      const pidMapFile = new JSONdb('pids.json');
      const pidMap = pidMapFile.get('pidMap');
      if(pidMap[showres.data.name]) {
